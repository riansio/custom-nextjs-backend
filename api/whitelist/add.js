// api/whitelist/add.js
import { kv } from "@vercel/kv";

// Retry wrapper to handle transient KV errors
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (!userId || !devProductId) return res.status(400).json({ error: "Missing userId or devProductId" });

    let record = await kv.get(`whitelist:${userId}`);
    ...
    await safeKVSet(`whitelist:${userId}`, record);
